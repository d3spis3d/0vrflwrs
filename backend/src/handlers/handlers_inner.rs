...
pub async fn read_answers(
    ...
    let answers = answers_dao
        .get_answers(question_uuid.question_uuid.to_owned())
        .await;
    ...
    match answers {
        ...
        Err(e) => {
            error!(
                "Error reading answers for {}: {}",
                &question_uuid.question_uuid, e
            );
            Err(HandlerError::default_internal_error())
        }
    }
...
pub async fn delete_answer(
    ...
    let result = answers_dao
        .delete_answer(answer_uuid.answer_uuid.to_owned())
        .await;

    if result.is_err() {
        error!(
            "Failed to delete answer {}: {:?}",
            answer_uuid.answer_uuid, result
        );
        return Err(HandlerError::default_internal_error());
    }
